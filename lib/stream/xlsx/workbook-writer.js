...
var CommentsWriter = require('./comments-writer');
...
WorkbookWriter.prototype = {
  ...
  _openStream: function(path) {
	...
	_commitComments: function() {
    var commitWorksheetComments = function(worksheetComments) {
			if (worksheetComments.isEmpty) {
      	return PromishLib.Promish.resolve();
			}

      if (!worksheetComments.committed) {
        return new PromishLib.Promish(function(resolve) {
          worksheetComments.stream.on('zipped', function() {
            resolve();
          });

          worksheetComments.commit();
        });
      }
      return PromishLib.Promish.resolve();
    };
    ...
    var promises = this._comments.map(commitWorksheetComments);
    if (promises.length) {
  ...
  _commitWorksheets: function() {
    var commitWorksheet = function(worksheet) {
			// HERE: gather comments and commit them
  ...
  commit: function() {
    // commit all worksheets and comments, then add suplimentary files
    return this.promise.then(() => this._commitWorksheets())
			//.then(() => this._commitComments())
  ...
  addWorksheet: function(name, options) {
    ...
    var worksheet = new WorksheetWriter({
      ...
      autoFilter: options.autoFilter
		...
		var comments = new CommentsWriter({
			id: id,
			workbook: this
		});
	...
	addComment(worksheetId, address, value) {
		this._comments[worksheetId].addComment(address, value)
	},
      ...
      if (worksheet) {
        ...
        relationships.push(
          {Id: worksheet.rId, Type: RelType.Worksheet, Target: 'worksheets/sheet' + worksheet.id + '.xml'},
					{Id: worksheet.rId, Type: RelType.Comments, Target: 'comments' + worksheet.id + '.xml' }
        );
      }
    });
    return new PromishLib.Promish(function(resolve) {
